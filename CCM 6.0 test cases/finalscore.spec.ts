//Exploring final score section of risk engine.
...
test('test', async ({ page }) => {
  ...
  await page.getByRole('cell', { name: 'Decision:FinalScore' }).click();
  await page.locator('.dmn-icon-decision-table').first().click();
  await page.getByRole('button', { name: 'View DRD' }).click();
  await page.locator('.dmn-icon-literal-expression').click();
  await page.getByRole('button', { name: 'View DRD' }).click();
  await page.getByRole('region').getByRole('link', { name: 'Risk Engines' }).click();
});