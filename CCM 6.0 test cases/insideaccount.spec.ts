...
test('test', async ({ page }) => {
  ...
  await expect(page.getByText('Type')).not.toHaveCount(0);
  await expect(page.getByText('Tenant')).not.toHaveCount(0);
  ...
  await page.getByText('AccountCEMTIRON MANHIGH').dblclick();
