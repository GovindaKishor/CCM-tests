...
test('test', async ({ page }) => {
  ...
  await page.getByText('Risk level').click({
    clickCount: 3
  });
  await page.getByText('Risk level').press('Control+c');
  await page.getByText('Risk score').click({
    clickCount: 3
  });
  await page.getByText('Risk score').press('Control+c');
});