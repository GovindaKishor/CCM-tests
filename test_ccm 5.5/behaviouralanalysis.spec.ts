...
test('test', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Choose date, selected date is 11 Jan 2023' }).click();
  await page.getByRole('gridcell', { name: '6', exact: true }).click();
  await page.getByRole('button', { name: 'Choose date, selected date is 10 Feb 2023' }).click();
